...
export function OrdersPage() {
  ...
  if (orders.length === 0) {
    return <EmptyState title="No orders yet" message="Place your first order from the products page." />;
  }
  ...
  return (
    <div className="section-stack">
      ...
      <div className="grid gap-4">
        {orders.map((order) => (
          <Link key={order.orderId} to={`/orders/${order.orderId}`} className="card-surface-hover block p-4">
