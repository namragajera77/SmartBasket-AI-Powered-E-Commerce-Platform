  ...
  return (
    <div className="section-stack">
      <Card className="space-y-4">
        ...
        >
          <Input label="Category Name" value={name} onChange={(event) => setName(event.target.value)} />
          <Input label="Description" value={description} onChange={(event) => setDescription(event.target.value)} />
          <div className="md:col-span-2">
            <Button type="submit" disabled={createCategoryMutation.isPending || name.trim().length < 2}>
              {createCategoryMutation.isPending ? "Creating..." : "Create Category"}
            </Button>
          </div>
        </form>
        {createCategoryMutation.isError ? <ErrorMessage message={createCategoryMutation.error.message} /> : null}
      </Card>

      <Card>
        <h2 className="mb-4 text-lg font-semibold text-gray-900">Existing Categories</h2>
        {categoriesQuery.isLoading ? <PageSkeleton rows={3} /> : null}
        {categoriesQuery.isError ? <ErrorMessage message={categoriesQuery.error.message} /> : null}
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {pagedCategories.map((category) => (
                <tr key={category.id} className="transition-all duration-200 hover:bg-gray-50">
