...
export function ProductDetailPage() {
  ...
  const updateReviewMutation = useMutation({
    mutationFn: ({ reviewId, nextRating, nextComment }) =>
  ...
  return (
    <div className="container-custom py-8 pb-24 space-y-12">
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        >
          <div className="mb-2">
            <span className="text-sm font-bold tracking-widest uppercase text-blue-600 bg-blue-50 px-3 py-1 rounded-full">
              {product.categoryName}
            </span>
          </div>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mt-4 leading-tight">
            {product.name}
          </h1>
          ...
          <div className="flex items-center gap-4 mt-4 mb-6">
            ...
            <a
              href="#reviews"
              className="text-gray-500 text-sm hover:text-blue-600 hover:underline transition-all"
            >
          ...
          <p className="text-gray-600 text-lg leading-relaxed mb-8">
            {product.description ??
              "Experience premium quality with our latest product. Designed to perfection to meet all your needs and expectations."}
          </p>
          ...
          <div className="flex items-center gap-6 py-4">
            <div className="text-sm text-gray-500 flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full ${product.stockQuantity > 0 ? "bg-green-500" : "bg-red-500"} shadow-[0_0_10px_rgba(34,197,94,0.5)]`}
              ></div>
          ...
          <div className="mt-6 flex gap-4">
            {role !== "Admin" && role !== "Delivery" ? (
              <button
                ...
                disabled={
                  addToCartMutation.isPending || product.stockQuantity === 0
                }
      ...
      >
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-3xl p-8 border border-blue-100 shadow-sm relative overflow-hidden">
          ...
          <p className="text-blue-900/80 leading-relaxed max-w-4xl text-lg">
            ...
            , customers appreciate the premium quality and fast performance. The
            average rating is{" "}
            <span className="font-bold text-blue-700">
              {ratingAverage.toFixed(1)} out of 5
            </span>
            . Buyers highlight excellent packaging and accurate item
            descriptions. Highly recommended for its price segment!
          </p>
        </div>
      </motion.div>
        ...
        <div className="lg:w-2/3 space-y-8">
          <div className="flex items-center justify-between border-b border-gray-100 pb-4">
            <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <MessageSquare className="w-6 h-6 text-blue-600" /> Customer
              Reviews
            </h2>
          </div>
          ...
          {reviewsQuery.isError ? (
            <ErrorMessage message={reviewsQuery.error.message} />
          ) : null}
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-5 h-5 ${i < review.rating ? "fill-yellow-400 text-yellow-400" : "fill-gray-100 text-gray-200"}`}
                      />
                    ))}
                  </div>
                </div>
                    ...
                    <button
                      ...
                      onClick={() =>
                        updateReviewMutation.mutate({
                          ...
                          nextRating: Math.min(5, review.rating + 1),
                          nextComment: review.comment,
                        })
                      }
                    >
                      Bump Rating +1
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`w-8 h-8 ${rating >= star ? "fill-yellow-400 text-yellow-400" : "fill-gray-100 text-gray-200"}`}
                        />
                      </button>
                    ))}
                  </div>
        ...
        <div className="lg:w-1/3">
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 sticky top-28">
            <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-purple-600" /> You might also
              like
            </h3>

            {recommendationQuery.isLoading ? <PageSkeleton rows={3} /> : null}
            {recommendationQuery.isError ? (
              <ErrorMessage message={recommendationQuery.error.message} />
            ) : null}
            {!recommendationQuery.isLoading &&
            (recommendationQuery.data?.length ?? 0) === 0 ? (
              <p className="text-gray-500 text-sm">
                More recommendations will appear based on shopping activity.
              </p>
            ) : null}
            ...
            <div className="space-y-4">
              {(recommendationQuery.data ?? []).map((item) => (
                <Link
                  ...
                  key={item.productId}
                  className="flex gap-4 p-3 rounded-2xl hover:bg-gray-50 border border-transparent hover:border-gray-100 transition-all group"
                >
                  <div className="w-20 h-20 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0 relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-gray-400 bg-gray-100 group-hover:scale-110 transition-transform">
                      Img
                    </div>
                  </div>
                  <div className="flex flex-col justify-center">
                    <h4 className="font-semibold text-gray-900 line-clamp-2 text-sm group-hover:text-blue-600 transition-colors">
                      {item.name}
                    </h4>
                ...
                (recommendationQuery.data?.length ?? 0) === 0 && (
                  <div className="space-y-4 opacity-50 pointer-events-none grayscale">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="flex gap-4 p-3 rounded-2xl border border-gray-50"
                      >
