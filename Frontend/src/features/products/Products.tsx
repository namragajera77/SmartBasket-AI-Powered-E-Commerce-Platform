...
export function ProductsPage() {
  ...
  const data = productsQuery.data ?? ({ page: 1, pageSize: defaultPageSize, totalItems: 0, items: [] } as const);
  ...
  return (
    <div className="container-custom py-8 pb-20">
      ...
      <motion.div initial="hidden" animate="visible" variants={fadeUp} className="mb-8">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Explore Products</h1>
        <p className="text-gray-500 max-w-2xl">Discover our curated collection of premium products. Filter, sort, and find exactly what you need.</p>
      </motion.div>

      <div className="flex flex-col lg:flex-row gap-8">
        
        {/* Sidebar Filters */}
        <motion.div initial="hidden" animate="visible" variants={fadeUp} className="lg:w-1/4 flex-shrink-0">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <button 
                    onClick={() => patchParams({ categoryId: undefined, page: 1 })}
                  ...
                  {(categoriesQuery.data ?? []).map((category) => (
                    <button
                      key={category.id}
                      onClick={() => patchParams({ categoryId: category.id, page: 1 })}
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => patchParams({ minPrice: e.target.value || undefined, page: 1 })}
                  />
                  ...
                  <input
                    ...
                    onChange={(e) => patchParams({ maxPrice: e.target.value || undefined, page: 1 })}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <select
                  className="input-field py-2 px-3 text-sm"
                  value={query.sortBy ?? ""}
                  onChange={(e) => patchParams({ sortBy: e.target.value || undefined, page: 1 })}
                >
              ...
              <button 
                ...
                onClick={() => setSearchParams({ page: "1", pageSize: String(defaultPageSize) })}
              >
                Reset Filters
              </button>
            </div>
        ...
        <div className="lg:w-3/4 flex-1">
          ...
          <div className="flex items-center justify-between mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
             <div className="text-sm text-gray-500">
               Showing <span className="font-bold text-gray-900">{data.items.length}</span> of <span className="font-bold text-gray-900">{data.totalItems}</span> products
             </div>
          ...
          {data.items.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm flex flex-col items-center justify-center">
              ...
              <button className="mt-6 btn-primary" onClick={() => setSearchParams({ page: "1", pageSize: String(defaultPageSize) })}>
                Clear Filters
              </button>
            </div>
          ) : (
            ...
            >
              {data.items.map((product) => (
                <motion.div variants={fadeUp} key={product.id} className="h-full">
                  <Link to={`/products/${product.id}`} className="h-full group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col items-start relative pb-4">
                    ...
                    <div className="p-5 flex flex-col flex-1 w-full z-20 bg-white">
                      <p className="text-xs font-semibold uppercase tracking-wider text-blue-600 mb-1.5">{product.categoryName}</p>
                      <h3 className="text-gray-900 font-semibold mb-2 line-clamp-2 leading-tight flex-1">{product.name}</h3>
          ...
          {data.totalItems > 0 && (
            <div className="mt-10 flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-100">
              <p className="text-sm text-gray-500">
                Page <span className="font-bold text-gray-900">{data.page}</span> of {Math.max(1, Math.ceil(data.totalItems / data.pageSize))}
              </p>
              <div className="flex items-center gap-2">
                <button 
