...
export function AdminOrdersPage() {
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ orderId, reason }) => rejectOrderAsAdmin(orderId, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-orders"] });
      queryClient.invalidateQueries({ queryKey: ["delivery-orders"] });
  ...
  const activeOrders = useMemo(
    () =>
      orders.filter(
        (order) => order.status !== "Delivered" && !isRejectedOrder(order),
      ),
    [orders],
  );
    ...
    <div className="section-stack">
      <Card>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Assign delivery staff and track status lifecycle.
        </p>
      </Card>

      {ordersQuery.isLoading ? <PageSkeleton rows={5} /> : null}
      {ordersQuery.isError ? (
        <ErrorMessage message={ordersQuery.error.message} />
      ) : null}
      {deliveryUsersQuery.isError ? (
        <ErrorMessage message={deliveryUsersQuery.error.message} />
      ) : null}
      ...
      <Card>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Use these IDs when assigning orders.
        </p>
        <div className="mt-3 max-h-44 overflow-auto rounded-lg border border-gray-200 bg-gray-50 p-3">
          {deliveryUsersQuery.isLoading ? (
            <p className="text-sm text-gray-500">Loading delivery staff...</p>
          ) : null}
          ...
          {(deliveryUsersQuery.data ?? []).length === 0 &&
          !deliveryUsersQuery.isLoading ? (
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {pagedOrders.map((order) => {
                const isLockedAssignment =
                  ["Assigned", "OutForDelivery", "Delivered"].includes(
                    order.status,
                        ...
                        <div className="flex min-w-64 items-end gap-2">
                          ...
                          <Button
                            ...
                            disabled={
                              ...
                              Number(deliveryInput[order.orderId] ?? 0) <= 0
                            }
                            onClick={() =>
                              assignMutation.mutate({
                                orderId: order.orderId,
                                deliveryBoyId: Number(
                                  deliveryInput[order.orderId] ?? 0,
                                ),
                              })
                            }
                          >
                      ...
                      <div className="mt-2 flex min-w-64 items-end gap-2">
                        ...
                        <Button
                          ...
                          disabled={
                            ...
                            (rejectInput[order.orderId] ?? "").trim().length < 3
                          }
                          onClick={() =>
      ...
      <Card>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Tracks why and when orders were rejected.
        </p>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {rejectedOrders.map((order) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-600">
                    {order.rejectedAtUtc
                      ? formatDate(order.rejectedAtUtc)
                      : "-"}
                  </td>
                  <td className="px-4 py-3 text-gray-700">
