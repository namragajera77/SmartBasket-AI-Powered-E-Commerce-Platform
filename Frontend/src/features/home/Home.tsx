...
import { Search, Monitor, Shirt, Home as HomeIcon, Sparkles, TrendingUp, ArrowRight, ShoppingBag } from "lucide-react";
...
const fallbackImage = "https://images.unsplash.com/photo-1550009158-9ebf69173e03?w=800&q=80";
...
function ProductCard({ id, name, price, image, isSmall = false }: HomeProductCard) {
  return (
    <motion.div variants={fadeUp} className="h-full">
      <Link to={`/products/${id}`} className="h-full group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col items-start relative pb-4">
        <div className={`w-full bg-gray-50 overflow-hidden relative ${isSmall ? 'h-48' : 'h-64'}`}>
          <ProductImage
            src={image}
            alt={name}
        ...
        <div className="p-5 flex flex-col flex-1 w-full relative z-20 bg-white">
          <h3 className="text-gray-900 font-semibold mb-1 line-clamp-2 leading-tight flex-1">{name}</h3>
...
export function HomePage() {
  ...
  const availableProducts = (productsQuery.data?.items ?? []).filter((product) => product.stockQuantity > 0);

  const mapProductToCard = (product: Product): HomeProductCard => ({
    id: product.id,
    name: product.name,
  ...
  const recommendedProducts = availableProducts.slice(4, 12).map(mapProductToCard);
    ...
    <div className="w-full space-y-24 pb-20">
        ...
        <div className="container-custom relative z-10 pt-10 pb-20">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div 
              ...
              className="max-w-2xl"
            >
              <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-100/50 text-blue-700 mb-6 font-medium text-sm backdrop-blur-sm border border-blue-200/50">
                <Sparkles className="w-4 h-4" />
                <span>AI-Powered Personalization Built-in</span>
              </motion.div>
              
              <motion.h1 variants={fadeUp} className="text-5xl md:text-6xl font-extrabold text-gray-900 tracking-tight leading-[1.1] mb-6">
                  Shop Smarter, <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">Not Harder.</span>
                </motion.h1>
              
              <motion.p variants={fadeUp} className="text-lg md:text-xl text-gray-600 mb-8 leading-relaxed max-w-lg">
                Discover the best premium products tailored just for you. Our AI engine finds exactly what you need in seconds.
              </motion.p>
              
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
                <Link to="/products" className="btn-primary text-lg px-8 py-4 rounded-xl flex items-center justify-center gap-2 group shadow-xl shadow-blue-500/20">
                  Shop Now <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </Link>
                <Link to="/search" className="btn-secondary text-lg px-8 py-4 rounded-xl flex items-center justify-center gap-2 group">
                  <Search className="w-5 h-5 text-gray-500 group-hover:text-blue-600 transition-colors" /> AI Search
                </Link>
              </motion.div>
            </motion.div>
      ...
      <section className="container-custom">
        ...
        >
          <motion.div variants={fadeUp} className="flex justify-between items-end mb-8">
             ...
             <Link to="/products" className="text-blue-600 font-medium hover:text-blue-700 flex items-center gap-1 group">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {categories.map((cat, i) => (
              <motion.div key={i} variants={fadeUp} className="h-full">
                <Link to={`/products?category=${cat.name}`} className="group flex flex-col items-center justify-center p-8 bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-blue-100 transition-all duration-300">
      ...
      <section className="container-custom">
         <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
          <motion.h2 variants={fadeUp} className="text-3xl font-bold text-gray-900 mb-8">Featured Collection</motion.h2>
          {featuredProducts.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-white p-8 text-gray-600">No products available right now.</div>
          ) : (
      ...
      <section className="container-custom">
        ...
        >
          ...
          <div className="relative z-10 max-w-3xl mx-auto space-y-8">
             ...
             <h2 className="text-4xl md:text-5xl font-bold">Find exactly what you want.</h2>
             <p className="text-lg text-gray-300">Type naturally. E.g., "I need a light laptop for video editing under $1500"</p>
      ...
      <section className="container-custom overflow-hidden pb-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer}>
          ...
          {recommendedProducts.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-white p-8 text-gray-600">No more products to recommend yet.</div>
          ) : (
