...
export function OrderDetailsPage() {
  ...
  if (orderQuery.isError || !orderQuery.data) {
    return (
      <ErrorMessage message={orderQuery.error?.message ?? "Order not found."} />
    );
  }
    ...
    <div className="section-stack">
      <Card className="space-y-3">
        ...
        <p className="text-sm text-gray-500">
          Placed at {formatDate(order.orderedAtUtc)}
        </p>
        <p className="text-sm text-gray-500">
        ...
        ["Assigned", "OutForDelivery"].includes(order.status) ? (
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
            ...
            <p className="mt-1 text-2xl font-bold tracking-[0.2em] text-amber-900">
              {order.deliveryOtp}
            </p>
            <p className="mt-1 text-xs text-amber-700">
              Show this OTP to your delivery person to complete handoff.
            </p>
          </div>
        ) : null}
        {canReject ? (
        ...
        {isRejectedOrder(order) ? (
          <div className="rounded-lg border border-rose-200 bg-rose-50 p-3">
            ...
            <p className="mt-1 text-sm text-rose-900">
              Rejected By: {order.rejectedBy ?? order.status}
            </p>
            <p className="mt-1 text-sm text-rose-900">
              Rejected At:{" "}
              {order.rejectedAtUtc ? formatDate(order.rejectedAtUtc) : "-"}
            </p>
            <p className="mt-1 text-sm text-rose-900">
      ...
      <Card className="space-y-3">
        ...
        <div className="grid gap-2">
          {order.items.map((item) => (
            <div
              key={item.productId}
              className="flex items-center justify-between rounded-xl border border-gray-200 p-3 transition-all duration-200 hover:shadow-md"
            >
              <div>
