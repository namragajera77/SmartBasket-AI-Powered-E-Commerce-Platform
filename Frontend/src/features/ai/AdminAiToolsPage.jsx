...
export function AdminAiToolsPage() {
  ...
  const descriptionMutation = useMutation({
    mutationFn: () =>
      generateDescription({ productName: name, category, keywords }),
    onSuccess: () => {
    ...
    <div className="section-stack">
      <Card className="space-y-4">
        ...
        <Button
          ...
          disabled={descriptionMutation.isPending || !name || !category}
        >
        ...
        {descriptionMutation.isError ? (
          <ErrorMessage message={descriptionMutation.error.message} />
        ) : null}
      ...
      <Card className="space-y-4">
        ...
        <div>
          ...
          {productsQuery.isError ? (
            <p className="mt-1 text-xs text-red-600">
              Could not load product list. You can still enter ID manually
              below.
            </p>
          ) : null}
        </div>
        ...
        <Button
          ...
          disabled={summaryMutation.isPending || Number(summaryProductId) <= 0}
        >
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">
          ...
          <p className="mt-1">
            {productsQuery.isLoading
              ? "Loading products..."
              : `${productsQuery.data?.items.length ?? 0} products loaded`}
          </p>
        ...
        {summaryMutation.isError ? (
          <ErrorMessage message={summaryMutation.error.message} />
        ) : null}
        {summaryMutation.data ? (
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">
            <p className="font-semibold text-gray-900">
              Product #{summaryMutation.data.productId}
            </p>
            <p className="mt-2">{summaryMutation.data.summary}</p>
            <p className="mt-2 text-xs text-gray-500">
              Reviews used: {summaryMutation.data.reviewCount}
            </p>
          </div>
        ) : null}
      </Card>
