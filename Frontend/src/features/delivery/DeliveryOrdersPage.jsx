...
export function DeliveryOrdersPage() {
  ...
  const verifyMutation = useMutation({
    mutationFn: ({ orderId, otp }) => verifyDeliveryOtp(orderId, otp),
    onSuccess: () => {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ orderId, status }) => updateDeliveryStatus(orderId, status),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["delivery-orders"] });
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ orderId, reason }) => rejectAssignedOrder(orderId, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["delivery-orders"] });
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <Button
                ...
                disabled={
                  verifyMutation.isPending ||
                  statusMutation.isPending ||
                  !otpValues[order.orderId]?.trim() ||
                  order.status === "Delivered"
                }
                onClick={() =>
                  verifyMutation.mutate({
                    orderId: order.orderId,
                    otp: otpValues[order.orderId] ?? "",
                  })
                }
              >
                Verify OTP
